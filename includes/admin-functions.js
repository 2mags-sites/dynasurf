 ...
 * Handles CSS background-image style hero sections
 */
function initHeroImageEditing() {
    ...
    heroSections.forEach(section => {
        ...
        const heroContent = section.querySelector('.hero-content-single, .hero-content');

        if (heroImage && editOverlay) {
            // Remove inline onclick and move overlay to section level
            editOverlay.removeAttribute('onclick');
            section.appendChild(editOverlay);

            // Position edit button in top-right corner
...
window.editHeroImage = function(overlayElement) {
    // Find the hero image div with data attributes
...
function saveAllChanges() {
    ...
    // Use .php extension for POST requests (avoids redirect issues)
